...
def content_generators(graph: nx.DiGraph, k: int) -> list:
    ...
    """
    if k * 10 > graph.number_of_nodes():
        warn("It does not make sense to have more than 10% of content generators")
...
def content_spreaders_pagerank(graph: snap.TNGraph, k: int, C=0.85, Eps=0.0001, MaxIter=100) -> list:
    ...
    # create df with each node and the corresponding pagerank
    ...
    for i in range(mc):
        ...
        while new_active:
            ...
            for node in new_active:
                # get weight of the edges with the neighborgs
